...
import com.kido.sdk.kdmonitor.presenter.KdDeblockObserver;
import com.kido.sdk.kdmonitor.presenter.KdNetworkObserver;
import com.kido.sdk.kdmonitor.presenter.KdScreenObserver;
import com.kido.sdk.kdmonitor.util.StatLog;

import java.util.List;

public class KdObserverPresenter implements KdNetworkObserver.INetworkListener, KdScreenObserver.IScreenListener,
        KdDeblockObserver.IKeyguardListener {

    /**
     * NetworkObserver
     */
    private KdNetworkObserver mNetworkObserver;
    /**
     * ScreenObserver
     */
    private KdScreenObserver mScreenObserver;
    /**
     * DeblockObserver
     */
    private KdDeblockObserver mKeyguardObserver;
    /**
     * isForeground
     */
    ...
    public KdObserverPresenter(ScheduleListener listener) {
    ...
    private void registerObserver(Context aContext) {
        ...
        if (mScreenObserver == null) {
            mScreenObserver = new KdScreenObserver(aContext, this);
        }
        mScreenObserver.start();

        if (mNetworkObserver == null) {
            mNetworkObserver = new KdNetworkObserver(aContext, this);
        }
        mNetworkObserver.start();

        if (mKeyguardObserver == null) {
            mKeyguardObserver = new KdDeblockObserver(aContext, this);
        }
    ...
    private synchronized void onForegroundChanged(Context aContext, boolean aIsForeground) {
        ...
        if (aIsForeground) {
            ...
            //切后台时数据上传
            KdStatSdk.getInstance(aContext).send();
            // 切后台停止计时
            scheduleStop();
    ...
    public void onNetworkConnected(Context aContext) {
        ...
        // 同步网络信息
        KdHeadrHandle.getHeader(aContext).setNetworkinfo(KdHeadrHandle.getNetWorkInfo(aContext));
        if (isForeground) {
